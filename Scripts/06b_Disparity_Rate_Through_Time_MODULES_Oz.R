...
load("Data/BayesTraits_TREES_Oz.RData")

# load the processed BayesTraits MODULE data
load("Data/BayesTraits_processed_MODULES_Oz.RData")

############################################################################

# extract just the Australian clade from the tree
oz.tree <- extract.clade(til.tree, node=63)

# and from the data
all.modules <- lapply(all.modules, function(x) x[which(rownames(x)%in%oz.tree$tip.label),])
...
save(head.corBM, body.corBM, tail.corBM, limb.corBM, file="Data/Disparity_processed_MODULES_Oz.RData")
...
slope.plot <- slope.plot + scale_y_break(c(0.5,3),scales=0.5)
...
rate.plot <- ggplot(RaT.df) +
               ...
               scale_color_brewer(palette="Spectral") +
               #geom_ribbon(aes(x=-time,ymin=`5%`,ymax=`95%`, fill=module), alpha=0.1) +
               #scale_fill_brewer(palette="Spectral") +
